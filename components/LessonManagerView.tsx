...
export const LessonManagerView: React.FC<LessonManagerViewProps> = ({ lessons, onSelectLesson, onUpdateLessons, onBack, progress }) => {
  ...
  return (
    <div className="min-h-screen bg-indigo-50 p-4 flex flex-col relative">
       ...
       <div className="max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-4 pb-20">
          {/* Add New Button */}
          <button 
             onClick={() => setIsEditing(true)}
             className="bg-white/50 border-4 border-dashed border-indigo-200 rounded-3xl p-6 flex flex-col items-center justify-center min-h-[160px] hover:bg-indigo-50 hover:border-indigo-400 transition-all text-indigo-400 gap-2"
          >
             <Plus size={48} />
          ...
          {lessons.map(lesson => (
             ...
             >
                ...
                <h3 className="text-xl font-bold text-gray-800 mb-1 pr-20">{lesson.title}</h3>
