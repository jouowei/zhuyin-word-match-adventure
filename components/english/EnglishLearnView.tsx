...
import { ArrowRight, Volume2, Snail } from 'lucide-react';
    ...
    if (item.kind === 'letter') {
      ...
      return (
        <div className="flex flex-col items-center gap-1">
          <span className={`font-english leading-none ${big ? 'text-[clamp(4rem,15vh,8rem)]' : 'text-[clamp(2.25rem,8vh,3.75rem)]'} font-bold text-gray-800`}>{info?.upper}{info?.lower}</span>
          <span className={`leading-none ${big ? 'text-[clamp(3rem,10vh,4.5rem)]' : 'text-[clamp(1.75rem,6vh,3rem)]'}`}>{item.emoji}</span>
          {info && <HighlightedKeyword keyword={info.keyword} letter={info.lower} className={`${big ? 'text-[clamp(1.75rem,5.5vh,2.25rem)]' : 'text-[clamp(1.1rem,3.5vh,1.5rem)]'} font-bold text-gray-700`} />}
        </div>
      );
    }
    return (
      <div className="flex flex-col items-center gap-1">
        ...
        <span className={`font-english ${big ? 'text-[clamp(2.5rem,9vh,3.75rem)]' : 'text-[clamp(1.5rem,5.5vh,2.25rem)]'} font-bold text-gray-800`}>{item.text}</span>
