import { ParentLockRecord } from '../types';
...
const readLock = (): ParentLockRecord | null => {
  const lock = familyStore.read('parentLock');
...
export const setParentPassword = (password: string, hint = ''): boolean => {
  ...
  const lock: ParentLockRecord = { salt, hash: sha256(salt + password), ...(hint.trim() ? { hint: hint.trim() } : {}) };
  return familyStore.write('parentLock', lock) && readLock()?.hash === lock.hash;
};
...
export const clearParentPassword = () => familyStore.remove('parentLock');
